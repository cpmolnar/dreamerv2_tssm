...
class Driver:
  ...
  def __call__(self, policy, steps=0, episodes=0):
    ...
    while step < steps or episode < episodes:
      ...
      if [len(np.array(actions[k]).shape) for k in actions][0]>2:
        actions = [
          {k: np.array(actions[k][i][0]) for k in actions}
...
class SequentialDriver:
  ...
  def __call__(self, policy, steps=0, episodes=0):
    step, episode = 0, 0
    [self._envs[i].reset() for i in range(len(self._envs))]
    while step < steps or episode < episodes:
      obs = {
          i: self._envs[i].reset()
          for i, ob in enumerate(self._obs) if ob is None or ob['is_last']}
      for i, ob in obs.items():
        ...
        self._eps[i] = [tran]
      # obs = {k: np.stack([o[k] for o in self._obs]) for k in self._obs[0]}
      state = {k: np.expand_dims(np.stack([i[k] for i in self._eps[0]], 0), 0) for k in self._eps[0][0].keys()}
      actions, self._state = policy(state, **self._kwargs)
      if [len(np.array(actions[k]).shape) for k in actions][0]>2:
        actions = [
          {k: np.array(actions[k][i][0]) for k in actions}
          for i in range(len(self._envs))]
